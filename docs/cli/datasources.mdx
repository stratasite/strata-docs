...
- A Strata project initialized (see [Projects](/cli/projects))
...
If you omit the datasource key, the CLI prompts you to choose one. You can search and filter tables, then select one (e.g. to copy the name for use with [Creating Tables](/cli/tables)).
...
When adding or editing a datasource, you can set a **tier** (`hot`, `warm`, or `cold`). Tiers influence query routing and performance. For a full explanation, see [Datasources](/semantic-model/datasources#tier-configuration).
...
3. **Set appropriate tiers** - Match performance characteristics (see [Datasources](/semantic-model/datasources#tier-configuration))
...
- [Creating Tables](/cli/tables) - Create semantic table models from datasource tables
- [Datasources (tiers, full config)](/semantic-model/datasources)
